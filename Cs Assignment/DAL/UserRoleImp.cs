...
namespace Cs_Assignment.DAL
{
    public class UserRoleImp : IUsers<Users, int>
    {
        ...
        {
            UserRoleImp.CheckingForEmail(user);
            UserRoleImp.CheckingForPassword(user);
            UserRoleImp.UsernameLengthChecking(user);
            UserRoleImp.EncodePasswordToBase64(user);
            UserRoleImp.RegisterUser(user);
            return true;
        }
